...
public class RoadSegment {
  ...
  // shape class lets us make rectangles and stuff and also has a contains method
  // so we can chekc if the car is on the road
  ...
  private Shape makeRoadShape() {
    switch (roadType) {
      ...
      case CURVE_LEFT:
        return new Arc2D.Double(-ROAD_WIDTH - ROAD_WIDTH / 2.0, -ROAD_WIDTH, ROAD_WIDTH * 2, ROAD_WIDTH * 2, 0, 90,
            Arc2D.PIE);
      case CURVE_RIGHT:
        return new Arc2D.Double(-ROAD_WIDTH / 2.0, -ROAD_WIDTH, ROAD_WIDTH * 2, ROAD_WIDTH * 2, 90, 90, Arc2D.PIE);
      case T:
        // found this thing called area on stack which lets u combine a bunch of tiny
        // shapes? idk its pretty cool
        Area tShape = new Area(
            new Rectangle2D.Double(-ROAD_WIDTH, -ROAD_WIDTH / 4.0, ROAD_WIDTH * 2, ROAD_WIDTH / 2.0));
  ...
  public boolean contains(double checkX, double checkY) {
    try {
      ...
      // applies the inverse transformation to the overall point to make the local
      // localVersion
      return roadShape.contains(localVersion);
    }
    // i was told that this might blow up a computer if we dont check for excpetions
    // (not all transforms can be inverted or something)
    catch (Exception e) {
      e.printStackTrace();
      return false;
    }
  ...
  public void draw(Graphics2D graphics) {
    ...
    // double dotRadius = 14;
    // graphics.fill(new Ellipse2D.Double(-dotRadius / 2.0, -dotRadius / 2.0,
    // dotRadius, dotRadius));
